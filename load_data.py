...
def load_Lai_data(dataset, basemethod='pca', ori=False):
    if dataset == 'ar':
        file_path = './data/Lai/ar_proj_data_adj.mat'
    elif dataset == 'feret':
        file_path = './data/Lai/feret_proj_data_adj.mat'
    else:
        # file_path = './data/Lai/humbi_proj_data_adj.mat'
        file_path = './data/Lai/humbinew_proj_data_adj.mat'

    mat_data = scipy.io.loadmat(file_path)

    adj = sp.csr_matrix(mat_data['adj'])
    adj = adj.astype(np.float32)
    data = np.array(mat_data['data'])
    ori_data = np.array(mat_data['ori_data'])
    if basemethod == 'pca':
        proj_mat = np.array(mat_data['pca_mat'])
    elif basemethod == 'lda':
        proj_mat = np.array(mat_data['lda_mat'])
    elif basemethod == 'lpp':
        proj_mat = np.array(mat_data['lpp_mat'])
    else:
        pass

    if basemethod in ['pca', 'lda', 'lpp']:
        features = data @ proj_mat
    else:
        features = data
    features = features.astype(np.float32)

    labels = np.array(mat_data['labels']).flatten().astype(np.int64) - 1
    idx_train = np.array(mat_data['idx_train']).flatten().astype(np.int64) - 1
    idx_stopping = np.array(mat_data['idx_stopping']).flatten().astype(np.int64) - 1
    idx_valtest = np.array(mat_data['idx_valtest']).flatten().astype(np.int64) - 1

    idx_np = {}
    idx_np['train'], idx_np['stopping'], idx_np['valtest'] = idx_train, idx_stopping, idx_valtest
    ...
    if ori:
        return graph, idx_np, ori_data
    else:
        return graph, idx_np


def load_deepface_data(dataset, base_path='./data/Lai/ar_proj_data_adj_facenet.mat'):
    if dataset == 'ar':
        file_path = './data/Lai/ar_proj_data_adj.mat'
    elif dataset == 'feret':
        file_path = './data/Lai/feret_proj_data_adj.mat'
    ...
    deep_data = scipy.io.loadmat(base_path)
    # deep_data 构建 knn的邻接矩阵

    features = np.array(deep_data['data'])
    features = features.astype(np.float32)
    adj = deep_data['adj']

    labels = np.array(deep_data['labels']).flatten().astype(np.int64) - 1
    idx_train = np.array(deep_data['idx_train']).flatten().astype(np.int64) - 1
    idx_stopping = np.array(deep_data['idx_stopping']).flatten().astype(np.int64) - 1
    idx_valtest = np.array(deep_data['idx_valtest']).flatten().astype(np.int64) - 1
